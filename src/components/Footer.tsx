...
const unitSelectClassNames = {
  control: ({ isFocused }: { isFocused: boolean }) =>
    `flex min-h-8 items-center rounded-md border bg-slate-100/95 px-2.5 text-slate-800 shadow-none transition sm:min-h-9 ${
      isFocused ? 'border-teal-500/50 ring-2 ring-teal-400/20' : 'border-slate-500/40'
    }`,
  valueContainer: () => 'gap-1 py-0',
  singleValue: () => 'text-xs font-medium text-slate-800',
  input: () => 'text-xs text-slate-800',
...
function UnitSelect({
  ...
  return (
    <label className="flex min-w-0 flex-col gap-0.5 text-left text-[0.65rem] font-medium text-slate-400/90">
      <span>{label}</span>
      <Select<UnitOption>
        unstyled
        options={options}
...
export default function Footer() {
  ...
  const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    if (data != null) setWeatherData(data);
  };
    ...
    <footer className="mt-auto border-t border-slate-600/40 bg-slate-800/95 text-slate-100">
      <div className="mx-auto max-w-7xl px-3 py-2 sm:px-6 sm:py-3 lg:px-8">
        <div className="flex items-center gap-1.5">
          <FontAwesomeIcon icon={faSliders} className="text-xs text-teal-400/80" />
          <h2 className="text-[0.7rem] font-medium text-slate-300">Display settings</h2>
        </div>
        <form
          className="mt-1.5 grid grid-cols-2 items-end gap-1.5 sm:grid-cols-2 sm:gap-2 lg:grid-cols-[1fr_1fr_1fr_auto]"
        ...
        >
          ...
          <button
            type="submit"
            className="col-span-2 h-8 rounded-md bg-teal-600/90 px-4 text-xs font-semibold text-white transition hover:bg-teal-500 active:scale-[0.98] sm:col-span-1 lg:col-span-1"
          >
            Apply
          </button>
        </form>
